...
std::shared_ptr<Journal> Journal::create(const string &dir, const string &jname, int serviceIdx, std::shared_ptr<IPageProvider> provider)
{
    std::shared_ptr<Journal> jp = std::shared_ptr<Journal>(new Journal());
...
void Journal::loadNextPage()
{
    ...
    {   // allocate new page
        std::shared_ptr<Page> newPage = pageProvider->getPage(directory, shortName, serviceIdx, curPage->getPageNum() + 1);
        // stop current page
        if (isWriting)
        {
            curPage->finishPage();
