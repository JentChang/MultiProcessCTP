...
void Page::finishPage()
{
    ...
    header->close_nano = NanoTimer::getInstance()->getNanoTime();
    header->frame_num = frameNum;
    header->last_pos = position;
    frame.setStatusPageClosed();
}

std::shared_ptr<Page> Page::load(const string &dir, const string &jname, short pageNum, bool isWriting, bool quickMode)
{
    ...
    if (header->status == JOURNAL_PAGE_STATUS_RAW)
    {
        ...
        header->start_nano = NanoTimer::getInstance()->getNanoTime();
        header->close_nano = -1;
    ...
    std::shared_ptr<Page> page = std::shared_ptr<Page>(new Page(buffer));
