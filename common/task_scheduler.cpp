...
namespace BackTraderCN
{
    ...
    {
        std::string cur_time_str = NanoTimer::getInstance()->parseNano(nano_, "%Y%m%d-");
        std::string date_time_str = cur_time_str + time_str;
        int64_t nano = NanoTimer::getInstance()->parseTime(date_time_str.c_str(), "%Y%m%d-%H:%M:%S");
        if (nano < nano_)
        {
            nano += NANOSECONDS_PER_DAY;
        }
